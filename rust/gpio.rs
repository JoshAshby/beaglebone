...
impl Pin {
  ...
  pub fn set_direction(&self, d: Direction) {
    ...
    if !(path.exists()) {
      fail!("Can't open pin direction file, is the pin exported?");
    }
    let a = match d {
      OUT => "out",
      IN => "in"
...
fn test_directions() {
  ...
  f.set_direction(IN);
  assert!((f.get_direction() as int) == 1);

  f.set_direction(OUT);
  assert!((f.get_direction() as int) == 0);
...
fn test_values() {
  ...
  f.set_value(HIGH);
  assert!((f.get_value() as int) == 1);

  f.set_value(LOW);
  assert!((f.get_value() as int) == 0);
